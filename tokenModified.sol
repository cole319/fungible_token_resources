...
contract ERC20 is IERC20 {
    ...
    function _burn(address account, uint256 amount) external {
        ...
        _beforeTokenTransfer(account, address(0), amountConverted);

        uint256 accountBalance = _balances[account];

        require(accountBalance >= amountConverted, "ERC20: burn amount exceeds balance");

        _balances[account] = accountBalance - amountConverted;
        _totalSupply -= amountConverted;

        emit Transfer(account, address(0), amountConverted);
    }
